...
class main:
 def main():
  ...
  if (hyu == '1') or (hyu == '01'):
   ...
   os.system("xdg-open https://youtube.com/channel/UCs4AaXaQYHQQhzV8UDZqVRw")
   main.main()
...
'x-email':'akunnuyul64@gmail.com',
 ...
 def vadershop(nom):
     ...
     if ('Gagal' in y) or ('hari' in y):
      print (f"{ken}[{kun}•{ken}] {ken}Spam {kan}({anj}CALL{kan}) {ken}Frame Work{kun} Gagal")
     else:
      print (f"{ken}[{kun}•{ken}] {ken}Spam {kan}({anj}CALL{kan}) {ken}Frame Work{kan} OK")
try:
 os.system("clear")
 bnr()
 menu=["Mulai","Subs My Channel","Info Tools","Update"]
